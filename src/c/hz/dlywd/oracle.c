...
extern int nDebugFlag; /* bkfxd.c定义 */
#endif
...
char sErrLogName[] = "log/bkfxd.log";
...
int dsConnectDatabase(int socket)
{
  ...
  {
    WriteErrLog("ConnectDatabase", "sUserName Get String error");
    close(socket);
    return 1;
  }
  ...
  {
    WriteErrLog("ConnectDatabase", "sPassWord Get String error");
    close(socket);
    return 1;
  }
  ...
  {
    WriteErrLog("ConnectDatabase", "Send Long error");
    close(socket);
    return 1;
  }
...
/* 
 * Function: dsConnectRemoteDatabase
 *
...
int dsConnectRemoteDatabase(int socket,char *sUserName,char *sPassWord)
{
  long nRetVal;
...
long ConnectDatabase(char *sUserName,char *sPassWord)
{
  ...
  char sDatabaseName[51] = "";
  char*s;
  strcpy(sDsUserName,sUserName);
  if(sPassWord==NULL||sPassWord[0]=='\0')
  {
    s = strchr(sDsUserName, '@');
    if (s)
    {
      strcpy(sDatabaseName, s);
    }
    s = strchr(sDsUserName, '/');
    if(s)
    {
    ...
    if(sDatabaseName[0])
      strcat(sDsUserName, sDatabaseName);
  }
  fprintf(stderr,"\nConnectDatabase: %s\n",sDsUserName);
...
int dsCloseDatabase(int socket)
{
  ...
  if(SendLongToSocket(socket,nRetVal))
  {
    WriteErrLog("CloseDatabase", "Send Long error");
  }
  close(socket);
  return 0;
}
...
long CloseRemoteDatabase(int socket)
{
  long nRetVal;
  ...
  if(SendLongToSocket(socket,(long)TASK_CLOSEDATABASE))
  {
    return -ERR_NO_TCPIP;
  }

  if(GetLongFromSocket(socket,&nRetVal))
  {
    return -ERR_NO_TCPIP;
  }
...
int dsGetSqlErrNo(int socket)
{
  ...
  {
    WriteErrLog("GetSqlErrNo", "Send Long error");
    close(socket);
    return 1;
  }
  return 0;
...
int dsGetSqlErrInfo(int socket)
{
  ...
  {
    WriteErrLog("GetSqlErrInfo", "Send String error");
    close(socket);
    return 1;
  }
  return 0;
...
int dsCommitWork(int socket)
{
  ...
  if(SendLongToSocket(socket,nRetVal))
  {
    WriteErrLog("CommitWork", "Send Long error");
    close(socket);
    return 1;
  }
  return 0;
...
int dsRollbackWork(int socket)
{
  ...
  if(SendLongToSocket(socket,nRetVal))
  {
    WriteErrLog("RollbackWork", "Send Long error");
    close(socket);
    return 1;
  }
  return 0;
...
int dsRunSql(int socket)
{
  ...
  {
    WriteErrLog("RunSql", "Get String error");
    close(socket);
    return 1;
  }
  ...
  if(SendLongToSocket(socket,nRetVal))
  {
    WriteErrLog(sOut, "Send Long error");
    close(socket);
    return 1;
  }
  return 0;
...
int dsOpenCursor(int socket)
{
  ...
  if(GetStringFromSocket(socket,sOut))
  {
    WriteErrLog("OpenCursor", "Get String error");    
    close(socket);
    return 1;
  }
...
int dsFetchCursor(int socket)
{
  ...
  {
    WriteErrLog("FetchCursor", "Get Long error");
    close(socket);
    return 1;
  }
...
*/
  ...
  if(SendLongToSocket(socket,nRetVal))
  {
    WriteErrLog("FetchCursor", "1 Send Long error");
    close(socket);
    return 1;
  }
  ...
  if(nRetVal==1)
  {
    ...
    {
      WriteErrLog("FetchCursor", "2 Send Long error");
      close(socket);
      return 1;
    }
    ...
    {
      WriteErrLog("FetchCursor", "Send Record error");
      close(socket);
      return 1;
    }
  }
...
int dsFetchCursorArray(int socket)
{
  ...
  if(GetLongFromSocket(socket,&nCursorId))
  {
    WriteErrLog("FetchCursorArray", "1 Get Long error");
    close(socket);
    return 1;
  }
  ...
  {
    WriteErrLog("FetchCursorArray", "2 Get Long error");
    close(socket);
    return 1;
  }
...
*/
  ...
  for(nLoop = 1; nLoop <= nRecNum; nLoop++)
  {
    ...
    if(SendLongToSocket(socket,nRetVal))
    {
      WriteErrLog("FetchCursorArray", "1 Send Long error");
      close(socket);
      return 1;
    }
    ...
    if(SendLongToSocket(socket,nColNum[nCursorId-1]))
    {
      WriteErrLog("FetchCursorArray", "2 Send Long error");
      close(socket);
      return 1;
    }
    ...
    if(SendRecordToSocket(socket,sTemp,l))
    {
      WriteErrLog("FetchCursorArray", "Send Record error");
      close(socket);
      return 1;
    }
  }
...
int dsCloseCursor(int socket)
{
  ...
  if(GetLongFromSocket(socket,&nCursorId))
  {
    WriteErrLog("CloseCursor", "Get Long error");
    close(socket);
    return 1;
  }
  ...
  if(SendLongToSocket(socket,nRetVal))
  {
    WriteErrLog("CloseCursor", "Send Long error");
    close(socket);
    return 1;
  }
  return 0;
...
int dsSetAutoCommitOn(int socket)
{
  ...
  if(SendLongToSocket(socket,nRetVal))
  {
    WriteErrLog("SetAutoCommitOn", "Send Long error");
    close(socket);
    return 1;
  }
  return 0;
...
int dsSetAutoCommitOff(int socket)
{
  ...
  if(SendLongToSocket(socket,nRetVal))
  {
    WriteErrLog("SetAutoCommitOff", "Send Long error");
    close(socket);
    return 1;
  }
  return 0;
...
int dsRunSelect(int socket)
{
  ...
  if(GetStringFromSocket(socket,sOut))
  {
    WriteErrLog("RunSelect", "Get String error");
    close(socket);
    return 1;
  }
             ...
             (sword) PARSE_NO_DEFER, (ub4) PARSE_V7_LNG))
  {
    ...
    if(SendLongToSocket(socket,nRetVal))
    {
      WriteErrLog(sOut, "1 Send Long error");
      close(socket);
      return 1;
    }
    return 0;
  ...
  if(sql_function!=FT_SELECT)
  {
    ...
    if(SendLongToSocket(socket,nRetVal))
    {
      WriteErrLog(sOut, "2 Send Long error");
      close(socket);
      return 1;
    }
    return 0;
  ...
  if((nColNum = describe_define(&cda)) <0)
  {
    ...
    if(SendLongToSocket(socket,nRetVal))
    {
      WriteErrLog(sOut, "3 Send Long error");
      close(socket);
      return 1;
    }
    return 0;
  ...
  if (oexfet(&cda, (ub4) 1, FALSE, FALSE))
  {
      if (cda.rc == NO_DATA_FOUND)
      {
        ...
        {
          WriteErrLog(sOut, "4 Send Long error");
          close(socket);
          return 1;
        }
        return 0;
      ...
      else
      {
        ...
        if(SendLongToSocket(socket,nRetVal))
        {
          WriteErrLog(sOut, "5 Send Long error");
          close(socket);
          return 1;
        }
        return 0;
  ...
  {
    WriteErrLog(sOut, "6 Send Long error");
    close(socket);
    return 1;
  }
  ...
  {
    WriteErrLog(sOut, "Get Long error");
    close(socket);
    return 1;
  }
  ...
  {
    WriteErrLog(sOut, "7 Send Long error");
    close(socket);
    return 1;
  }
...
*/
      ...
      default:
        ...
        l+=n;
/*
        if(SendLongToSocket(socket,(long)TYPE_STRING))
        {
          close(socket);
          return 1;
        }
...
*/
  ...
  if(SendRecordToSocket(socket,sTemp,l))
  {
    WriteErrLog(sOut, "Send Record error");
    close(socket);
    return 1;
  }
  return 0;
...
int dsRunProc(int socket)
{
  ...
  if(GetStringFromSocket(socket,sOut))
  {
    WriteErrLog("RunProc", "1 Get String error");
    close(socket);
    return 1;
  }
  ...
  for(i=1;i<=nOutputArgNum;i++)
  {
    ...
    {
      WriteErrLog(sOut, "2 Get String error");
      close(socket);
      return 1;
    }
  }
  ...
  for(i=1;i<=nOutputArgNum;i++)
  {
    ...
    {
      WriteErrLog(sOut, "Send String error");
      close(socket);
      return 1;
    }
  }
...
#ifdef DATASERVER
int WriteErrLog(char* sText, char* sErrInfo)
{
  FILE* fpLog;
  struct stat statFile;

  if(!stat(sErrLogName, &statFile))
  {
  ...
  if((fpLog=fopen(sErrLogName,"a+"))!=NULL)
  {
    time_t	now;
    (void) time(&now);
    fprintf(fpLog, "\n%s", ctime(&now));
