...
int GetMac(const char* caInBuf, int iInLen,unsigned int caMacKey[8],char* caMac)
{                         
    /***                        return code: 0  成功
                                             -1 其他错误
    ***/                  
    ...
    ptr_tmpmacbuffer = (char *)malloc(iInLen + 1);
...
long  EnCrypt(const char* caInBuf,const char* caKey,char *caOutBuf)
{
    char caTmpBuffer[17];
    int i;
...
long DeCrypt(const char* caInBuf,const char* caKey,char* caOutBuf)
{
