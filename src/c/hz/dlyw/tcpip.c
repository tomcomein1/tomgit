...
char sLocalIPAddr[16];
char sRemoteIPAddr[16];

/*------------------------------------------------------------------------
 * ConnectRemote - TCP client for DLYW service
 *------------------------------------------------------------------------
 *
 * 返回值：> 0 建立与远方服务器的socket
 *         < 0 出错
 *            -1 建立连接出错
 *            -ERR_NO_ENCRYPT 密钥验证出错
 *            -ERR_NO_TCPIP 传输出错
 *            -ERR_NO_PROCESS 服务器进程数超出 
 *            -ERR_NO_CONNECT 远方主机不能连接数据库
 */
...
int ConnectRemote(char* sRemoteHost, char* sService)
{
  ...
  nSocket = connectTCP(sRemoteHost, sService);
  if(nSocket < 0)
  {
    strcpy(sSysErrInfo, sTcpipErrInfo);
    return -1;
  }

  if(GetFromSocket(nSocket, "%ld %ld %ld %ld", &one, &two, &three, &four))
  {
    shutdown(nSocket,2);
    nSocket = 0;
    strcpy(sSysErrInfo, "密钥验证出错.");
    return -ERR_NO_ENCRYPT;
  }
    
  nEncryptKey = (one + 81) * (one + 81) + (two + 81) * (two + 81) + 
    (three + 81) * (three + 81) + (four + 81) * (four + 81) + 
    (one / 81 + 1) * (two / 81 + 1) * (three / 81 + 1) * (four / 81 + 1);

  if(SendToSocket(nSocket, "%ld %ld", nEncryptKey, 1L))
  {
    shutdown(nSocket,2);
    nSocket=0;
    strcpy(sSysErrInfo, "网络出错.");
    return -ERR_NO_TCPIP;
  }

  if(GetLongFromSocket(nSocket,&nRetVal))
  {
    shutdown(nSocket,2);
    nSocket=0;
    strcpy(sSysErrInfo, "网络出错.");
  ...
  {
    nSocket = 0;
    strcpy(sSysErrInfo, "服务器进程数超出.");
    return -ERR_NO_PROCESS;
  }

  if(GetDoubleFromSocket(nSocket,&dVersion))
  {
    shutdown(nSocket,2);
    nSocket=0;
    strcpy(sSysErrInfo, "网络出错.");
  ...
  if(SendToSocket(nSocket, "%ld %ld", 1L, (long)TASK_CONNECTDATABASE))
  {
    shutdown(nSocket,2);
    nSocket=0;
    strcpy(sSysErrInfo, "网络出错.");
  ...
  if(nRetVal < 0)
  {
    strcpy(sSysErrInfo, "远方主机不能连接数据库");
...
{
  long nRetVal;
  if(SendLongToSocket(nSocket, (long)TASK_CLOSEDATABASE))
  {
    shutdown(nSocket,2);
    nSocket=0;
    strcpy(sSysErrInfo, "网络出错.");
...
*/
  ...
  
  {
    ...
    strcpy(sLocalIPAddr, inet_ntoa(sa.sin_addr));
    len=sizeof(struct sockaddr_in);
    getpeername(s, (struct sockaddr*)&sa,&len);
    strcpy(sRemoteIPAddr, inet_ntoa(sa.sin_addr));
  }
...
int GetStringFromSocket(const int socket,char *sValue)
{
  ...
  n = ReadSocket(socket, sValue, (int)nLen);
  if (n < nLen)
    return 1;
