...
public class ParaseXML {
	...
	public String listMap2xml(List<Map<String, Object>> list, String listRoot,
			String mapRoot) {
		...
		try {
			...
			Logger.getLogger(ParaseXML.class.getName()).log(Level.SEVERE, null,
					ex);
		} finally {
			try {
				sw.close();
			} catch (IOException ex) {
				Logger.getLogger(ParaseXML.class.getName()).log(Level.SEVERE,
						null, ex);
			}
		}
